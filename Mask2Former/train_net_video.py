...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if dataset_name[:5] == 'ytvis':
            return YTVISEvaluator(dataset_name, cfg, True, output_folder)
        else:
            return ScanNetVideoEvaluator(dataset_name, output_folder)

    @classmethod
    def build_train_loader(cls, cfg):
        dataset_name = cfg.DATASETS.TRAIN[0]
        if dataset_name[:5] == 'ytvis':
            os.environ['RUN_ON_SCANNET'] = 'False'
            mapper = YTVISDatasetMapper(cfg, is_train=True)
        else:
            mapper = ScanNetMapper(cfg, is_train=True)
...
def main(args):
    ...
    os.environ['DEMO'] = str(cfg.DEMO.GENERATE_VIDEO)
