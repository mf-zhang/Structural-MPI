    ...
    def __init__(
        ...
        *,
        ...
        if os.environ['OLD_MPI'] == 'True':
            ...
            self.scales = range(0,4) # range(0,1)
            # MINE decoder
    ...
    def forward_features(self, features):
        ...
        for o in out:
            if num_cur_levels < self.maskformer_num_feature_levels:
                ...
                num_cur_levels += 1
        if os.environ['OLD_MPI'] == 'False':
            return self.mask_features(out[-1]), out[0], multi_scale_features
        if os.environ['OLD_MPI'] == 'True':
            # zmf
            S = 16
            # encoder_out = features['res5'] # b, 2048, 8, 12
            # conv_down1 = self.conv_down1(self.downsample(encoder_out)) # b, 2048, 8, 12 - b, 2048, 4, 6  - b, 512,  4, 6
            # conv_down2 = self.conv_down2(self.downsample(conv_down1))  # b,  512, 4, 6  - b,  512, 2, 3  - b, 256,  2, 3
            # conv_up1 = self.conv_up1(self.upsample(conv_down2))        # b,  256, 2, 3  - b,  256, 4, 6  - b, 256,  4, 6
            # conv_up2 = self.conv_up2(self.upsample(conv_up1))          # b,  256, 4, 6  - b,  256, 8, 12 - b, 2048, 8, 12
            conv_up2 = features['res5'].float()

            # repeat / reshape features
            B, C_feat, H_feat, W_feat = conv_up2.size()
            ...
            # conv_up2 = torch.cat((feat_tmp, disparity_BsCHW), dim=1) # B*S, 2048+21, 8, 12
            conv_up2 = feat_tmp # B*S, 2048, 8, 12

            # repeat / reshape features
            mapping = ['conv1_out','res2','res3','res4','res5']
            for i, feat_name in enumerate(mapping):
                _, C_feat, H_feat, W_feat = features[feat_name].size()
                feat_tmp = features[feat_name].unsqueeze(1).expand(B, S, C_feat, H_feat, W_feat) \
                    .contiguous().view(B * S, C_feat, H_feat, W_feat)
                # disparity_BsCHW = disparity.repeat(1, 1, H_feat, W_feat)
                # features[i] = torch.cat((feat_tmp, disparity_BsCHW), dim=1) # B*S, c+21, h/x, w/x
                features[feat_name] = feat_tmp # B*S, c, h/x, w/x
            ...
            output_nonplane_rgba_list = [outputs[("disp", i)] for i in self.scales]

            return self.mask_features(out[-1]), out[0], multi_scale_features, output_nonplane_rgba_list
